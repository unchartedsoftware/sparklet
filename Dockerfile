...
#
# Usage:
# $ docker build -t uncharted/sparklet:2.4.4 .
# $ docker run -p 8080:8080 -it uncharted/sparklet:2.4.4
...
RUN \
  ...
  curl http://apache.mirror.gtcomm.net/spark/spark-2.4.4/spark-2.4.4-bin-hadoop2.6.tgz > spark.tgz && \
...
ENV PATH /opt/spark-2.4.4-bin-hadoop2.6/bin:$PATH
